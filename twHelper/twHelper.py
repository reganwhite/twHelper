...
class twHelper:
    ...
    def findPlayerTribe(self, userID):
        """Find the userID of a username."""
        index = [i for i, names in enumerate(self.tribesCol[1]) if userID in names]
    ...
    # -------------------------------------------------------------------------------------------
    @staticmethod
    def parseCoordinateList(userInput):
    ...
    # -------------------------------------------------------------------------------------------
    @staticmethod
    def farmRankProcessList(list, order=False, descending=True, bbcode=True, title="ERROR"):
        listfix = []
        for i in list:
            if len(i) > 1:
                listfix.append(i)
    ...
    def getInDayInfo(self, name, type):
        if type == "Plunders":
            # get loot_vil data
            temp = requests.get(
                "https://" + self.worldName + ".tribalwars.net/guest.php?screen=ranking&mode=in_a_day&type=loot_vil&name=" + name)
            ...
            # get scavenge data
            temp = requests.get(
                "https://" + self.worldName + ".tribalwars.net/guest.php?screen=ranking&mode=in_a_day&type=scavenge&name=" + name)
            text = temp.text.replace('<span class="grey">.</span>', "")
            list = re.findall('lit-item\">(.*)</td>', text)
            self.scavList.append([name] + list)
        else:
            # get loot_res data
            temp = requests.get(
                "https://" + self.worldName + ".tribalwars.net/guest.php?screen=ranking&mode=in_a_day&type=loot_res&name=" + name)
            text = temp.text.replace('<span class="grey">.</span>', "")
            list = re.findall('lit-item\">(.*)</td>', text)
        ...
        for i in range(0, len(temp)):
            ...
            for j in range(0, len(self.speeds)):
                ...
                if bbcode:
                    ...
                    sA = timeLaunchString
                    sB = "Launch " + unitName.ljust(3) + " from [village]" + temp[i][
                        1] + "[/village] at " + timeLaunchString
    ...
    def getWorldInfo(self):
        ...
        if not os.path.isfile(self.storageDirectory + self.worldName + "Villages.txt") or (
                time.time() - os.path.getmtime(self.storageDirectory + self.worldName + "Villages.txt")) / 60 >= 60:
            # load villages
            ...
            # load ODD
            print("Downloading tribe information... ")
            map = requests.get("https://" + self.worldName + ".tribalwars.net/map/kill_def.txt")
            tribes = open(self.storageDirectory + self.worldName + "kill_def.txt", "w")
            tribes.write(map.text)
            tribes.close()
...
class twHelperMain(twHelperGUI.mainFrame):
    ...
    def __init__(self, parent):
        ...
        self.coordIgnoreEvtText = False

        # snipe timer variables
        self.villageListStorage1 = []
        self.villageListStorage2 = []

        # coordGenerator variables
        self.villageListStorage3 = []
    ...
    def snipeUpdateSearch(self, event):
        ...
        if currentText != "" and len(
                currentText) >= 4:  # check that the input isnt empty, and is 4 characters or longer
            found = False
            for choice in self.helper.playersCol[1]:  # for all usernames
                ...
                if lowerChoice.startswith(currentText.lower()):  # case insensitive search
                    ...
                    self.snipeSearch.SetValue(choice)  # apply the value to the current prediction

                    # highlight the part of the prediction that we haven't typed yet
                    ...
                    # Find Villages
                    userID = self.helper.findUserInfo("idFromUser",
                                                      currentText)  # get user id of current prediction/choice
    ...
    def attackTimerThread(self):
        ...
        timeString = str(attackDate.day) + "." + str(attackDate.month + 1) + "." + str(attackDate.year) + " " + str(
            attackTime.hour) + ":" + str(attackTime.minute) + ":" + str(attackTime.second) + "." + str(milliseconds)
        # get the timing string
        string = self.helper.getSnipeString2(target, timeString, self.snipeBBcode.IsChecked(),
                                             self.snipeSort.IsChecked(), sendLocationCoordinates)
    ...
    def fillMemberList(self):
        ...
        string = self.farmRankList(self.farmSearchAbbrev.GetValue(), order=order, descending=descending, type=typ,
                                   bbcode=check)
    ...
    def farmRankList(self, tribeAbbrev, order=False, descending=True, bbcode=True, type="Hauls"):
        ...
        for i in range(0, len(members)):
            ...
            self.twHelperLog.SetValue(
                "[" + str(i + 1) + "/" + str(len(members)) + "]\n")  # update the progress value in the log
            self.helper.getInDayInfo(members[i], type)  # get info for member[i]

        self.twHelperLog.AppendText(
            "Finished getting info for " + tribeAbbrev)  # pri nt that we are finished to the log

        # turn our list of member ranks into a string
        if type == "Plunders":
            string = self.helper.farmRankProcessList(self.helper.plunderList, order=order, descending=descending,
                                                     bbcode=bbcode, title="Total Plunders")
        elif type == "Scavenger":
            string = self.helper.farmRankProcessList(self.helper.scavList, order=order, descending=descending,
                                                     bbcode=bbcode, title="Total Scavenged")
        else:
            string = self.helper.farmRankProcessList(self.helper.haulList, order=order, descending=descending,
                                                     bbcode=bbcode, title="Total Haul")
    ...
    def coordSearchClear(self, event):
        self.coordSearch.Clear()
        self.coordIgnoreEvtText = True

    def coordUpdateSearch(self, event):
        if self.coordIgnoreEvtText:
            self.coordIgnoreEvtText = False
            return
        ...
        if currentText != "" and len(
            ...
            for choice in self.helper.playersCol[1]:  # for all usernames
                ...
                if lowerChoice.startswith(currentText.lower()):  # case insensitive search
                    # Set the search prediction
                    self.coordIgnoreEvtText = True
                    self.coordSearch.SetValue(choice)  # apply the value to the current prediction

                    # highlight the part of the prediction that we haven't typed yet
                    self.coordSearch.SetInsertionPoint(len(currentText))
                    self.coordSearch.SetSelection(len(currentText), len(choice))

                    # Set the tribe abbreviation
                    self.coordTribe.SetValue(self.helper.findUserInfo("tribeFromUser", choice))

                    # Find Villages
                    userID = self.helper.findUserInfo("idFromUser",
                                                      currentText)  # get user id of current prediction/choice
                    villages = self.helper.findVillage(userID)  # get all villages belonging to the choice
                    self.villageListStorage3 = self.helper.rowToVillage(villages)  # get formatted village list

                    # process formatted village list so that we only get a pretty string
                    queryVillages = []
                    for i in range(0, len(self.villageListStorage3)):
                        queryVillages.append(self.villageListStorage3[i][2])  # append the "NAME (XXX|YYY)" string
                    self.coordVillage.SetItems(queryVillages)  # send to output window
    ...
    def coordinateFinder(self, villageLocation, vilType = "Barbs", distMin = 0, distMax = 999999, ptsMin = 0, ptsMax = 20000, oddMin = 0):
        strOut = ""
        for i in range(0,len(self.helper.villagesCol[2])):
            dist = self.helper.distanceCalc(villageLocation, str(self.helper.villagesCol[2][i] + "|" + self.helper.villagesCol[3][i]))
            if distMin <= dist <= distMax:
                if ptsMin <= int(self.helper.villagesCol[5][i]) <= ptsMax:
                    if vilType == "Barbs":
                        if self.helper.villagesCol[4][i] == "0":
                            #print(str(self.helper.villagesCol[2][i] + "|" + self.helper.villagesCol[3][i]))
                            strOut = strOut + str(self.helper.villagesCol[2][i] + "|" + self.helper.villagesCol[3][i]) + " "
                    elif vilType == "Players":
                        if self.helper.villagesCol[4][i] != 0:
                            #print(str(self.helper.villagesCol[2][i] + "|" + self.helper.villagesCol[3][i]))
                            strOut = strOut + str(self.helper.villagesCol[2][i] + "|" + self.helper.villagesCol[3][i]) + " "
                    else:
                        if self.helper.villagesCol[4][i] == 0:
                            ...
                            kappa = "this doesnt work yet"
        self.generatorOutput.SetValue(strOut)

    def coordVillageSelect(self, event):
        event.Skip()

    def coordVillageRunGenerate(self, event):
        selection = self.helper.parseCoordinateList(self.coordVillage.GetString(self.coordVillage.GetSelection()))
        print(selection)
        # self.coordVillage.GetString(self.coordVillageType.GetSelection())
        self.coordinateFinder(selection[0], vilType = "Barbs", distMin = int(self.coordDistMin.GetValue()),
                              distMax = int(self.coordDistMax.GetValue()), ptsMin = int(self.coordPointsMin.GetValue()),
                              ptsMax = int(self.coordPointsMax.GetValue()), oddMin = 0)
        event.Skip()
